...
import math
import numpy as np  # Используем NumPy
...
position = np.array([0.0, 0.0, 0.0])  # NumPy массив
velocity = np.array([0.03, 0.05, 0.02])
box_size = 4.0
texture_id = 0
...
def display(window):
    ...
    glPushMatrix()
    pos = position.tolist()  # преобразуем обратно в список для OpenGL
...
def spherical_to_cartesian(r, theta, phi):
    ...
    z = r * math.cos(theta)
    return np.array([x, y, z]).tolist()  # преобразуем в list для glVertex3fv
